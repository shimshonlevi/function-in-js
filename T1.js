...
//   console.log(oldPeople);
  ...
  const johnsFriendsNames = data
  .filter(person => person.name === "John Doe") 
  .map(person => person.friends.map(friend => friend.name))
  .flat();

// console.log(johnsFriendsNames);
  
  // Exercise 2: Use the map method to get the full addresses (street, city, state, and zip) of all the people in the data array
  const fullAddresses = data.map(person => `${person.address.street}, ${person.address.city}, ${person.address.state}, ${person.address.zip}`);
...
//   console.log(allHobbies);
  ...
  const liveInChicago = data.find(person => person.address.city === "Chicago");
